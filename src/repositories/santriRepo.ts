...
export interface SantriFilter {
  tahapHafalan?: TahapHafalan | string;
  search?: string;
  ortuId?: number;
  page: number;
  limit: number;
...
export const updateSantriAndUser = async (id: number, data: SantriUpdateData) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const { email, password, ...restData } = santriData as any;
    ...
    const updatePayload: any = { ...restData, ...(noInduk && { noInduk }) };
    if (ortuId) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (santri.fotoProfil && santri.fotoProfil.includes('/public/santri')) {
      try {
        ...
        console.error('Gagal menghapus file foto profil:', error);
      }
    }
