...
export const sendAccountEmail = async ({ to, name, email, password, role }: SendAccountEmailParams) => {
  ...
  const htmlContent = `
    ...
    <p>Peringatan: jangan pernah memberikan password ini kepada siapapun.</p>
    <p>Terima kasih.</p>
  `;
...
export const sendUpdateEmail = async ({ to, name, oldEmail, newEmail, role, passwordChanged, newPassword }: any) => {
