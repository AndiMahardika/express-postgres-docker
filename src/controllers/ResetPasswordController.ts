...
export const ResetPasswordController = {
  async forgotPassword(req: Request, res: Response) {
    ...
    try {
      ...
      res.status(200).json({ message: 'Password reset link has been sent to your email.', status: 200, data: result });
    } catch (error) {
  ...
  async resetPassword(req: Request, res: Response) {
    ...
    try {
      ...
      res.status(200).json({ message: 'Password has been reset successfully.', status: 200, data: result });
    } catch (error) {
      if (error instanceof Error) {
        return res.status(400).json({ error: error.message });
      }
  ...
  async verifyOldPassword(req: AuthRequest, res: Response) {
    ...
    if (isMatch) {
      res.status(200).json({ message: 'Old password verified successfully.', status: 200 });
    } else {
      res.status(400).json({ error: 'Incorrect old password.', status: 400 });
    }
  } catch (error) {
    if (error instanceof Error) {
  ...
  async resetPasswordWithOldPassword(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      res.status(200).json({ message: 'Password has been updated successfully.', status: 200, data: { userId } });
    } catch (error) {
      if (error instanceof Error) {
        return res.status(500).json({ error: error.message });
      }
